// ============================================================
// score.js  v2.2 — 형태정확성 Group A/B 구조 복잡도 차등 점수
// ============================================================
// [v2.1 → v2.2 주요 변경]
// 1. Group A/B 분류:  형태정확성 Step 1 내에 구조 복잡도 그룹 도입
//    Group A (あ·え·お): 기본 30점, 교차점·루프·Aspect Ratio 조건 적용
//    Group B (い·う):   기본 28점, 획 간 간격·평행도 조건 적용
// 2. 5대 루브릭 명칭 수정: '4대' 오기 수정
// ────────────────────────────────────────────────────────────
// [v2.0 → v2.1 누적 변경]
// 1. Safe Zone 확대:   ±15% → ±20%
// 2. 균형비율 floor:   최소 5점 보장 신설
// 3. 폭 측정 일관성:   캔버스 대비 자형 절대 비율 기준 명시
// 4. 루프 평가 세분화: 둥근 원형 / 각진 루프 / 열린 C자 3단계
// ────────────────────────────────────────────────────────────
// [v1.x → v2.0 누적 변경]
// 1. 필순 가산 모델:  획 수 확인(12pt) + 순서 확인(+8pt)
// 2. 채점 프롬프트:   가산제 전면 재작성
// 3. 급소 함수 신설:  글자별 Critical Points 분리
// 4. 후처리 단순화:   floor 3개(끝맺음·획방향·균형비율)
// 5. thinkingBudget:  2048 → 1024
// ============================================================
...
[Step 1 — 인식 성공 여부 + 구조 복잡도 차등]

  ━ 먼저 글자의 구조 그룹을 판별하고, 그룹에 맞는 기본점과 조건을 적용한다.

  ┌─ Group A: 복합 구조 (교차점·루프 포함) — あ·え·お (현재 지원 글자 기준)
  │   기본 인식점: 30점
  │   교차점·루프가 없거나 Aspect Ratio 왜곡이 심한 경우 상한 22점으로 강제
  │
  │   [조건 A-1] Aspect Ratio 왜곡 감지
  │   자형의 가로:세로 비율 정상 범위: 1:0.8 ~ 1:1.5
  │   → 세로가 가로의 1.5배 초과 (세로 압착): 기본점 -8 (30→22점)
  │   → 가로가 세로의 1.5배 초과 (가로 늘어남): 기본점 -8 (30→22점)
  │   → 왜곡 없음: 감산 없음
  │   ★ 측정: 획 중심 경로 기준. 끝의 삐침·흘림은 제외.
  │
  │   [조건 A-2] 삼각형 여백 검사 — あ 전용
  │   1·2·3획 교차 지점에 삼각형 열린 여백이 형성되어 있는가?
  │   → 삼각형 여백 명확히 보임: 감산 없음
  │   → 여백이 극히 좁거나 선들이 뭉쳐 여백 없음: 기본점 -5
  │   → 3획이 교차점 미통과 (삼각형 자체 불성립): 기본점 -8
  │   ★ 작더라도 삼각형 공간이 시각적으로 존재하면 감산 없음.
  │
  └─ Group B: 단순 구조 (교차점·루프 없음) — い·う (현재 지원 글자 기준)
      기본 인식점: 28점 (복잡도가 낮으므로 출발점을 차등 적용)
      교차점·루프 검사 없음. 대신 획 간 관계 검사 적용.

      [조건 B-1] 획 간 간격·평행도 검사
      두 획 사이의 간격이나 평행 관계가 유지되는가?
      → 간격 균형 유지 + 방향 평행: 감산 없음 (28점 유지)
      → 간격이 지나치게 좁거나 넓어 글자 구조 붕괴: -5점 (28→23점)
      → 두 획이 교차하거나 완전히 겹침: -8점 (28→20점)

  ※ い·うに Aspect Ratio 조건(A-1)은 적용하지 않는다.
     두 글자는 원래 세로로 긴 자형이므로 비율 왜곡 기준이 다르다.

  ▸ 모든 그룹 공통: '${target}'로 인식 불가 시 → 0~15점으로 세분화
    (Group A 30점 / Group B 28점 출발점 모두 무효화하고 0~15점 적용)

[Step 2 — 곡선·루프 완성도] (0~10점 추가)
  → 부드럽고 둥근 원형 루프:                           +7~10점
  → 닫혀 있지만 각지거나 울퉁불퉁한 루프(삼각·사각형): +3~5점  ← 닫혔어도 원형 아니면 감산
  → 곡선 없이 직선이거나 루프가 완전히 열린 C자:        +0~2점

[루프 평가 핵심 규칙]
  '닫혀 있으면 완성'이 아닙니다. **모양이 둥근가**를 반드시 확인하세요.
  → 루프가 시각적으로 원/타원에 가까워야 +7점 이상 부여 가능합니다.
  → 루프가 삼각형·사각형·마름모 형태로 각져 있으면 +3~5점 구간을 적용하세요.
  → 루프가 완전히 열려 C자·반원으로만 보이면 +0~2점입니다.
  시작점과 끝점이 완전히 겹치지 않아도 **시각적으로 둥글면** 감산 없음.
  단, **각진 루프**는 설령 닫혀 있어도 반드시 감산하세요.
